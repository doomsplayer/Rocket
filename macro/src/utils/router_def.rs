...
impl RouterDef {
    pub fn new(method: Option<Method>, meta: Expr, item: Item) -> Result<Self> {
        ...
        if kvmap.len() != 0 {
            let params = kvmap.keys().map(|key| key.clone()).collect();
            bail!(RouteUnknownParam(params))
        }
    ...
    fn parse_path(expr: &Expr) -> Result<(String, Option<String>)> {
        ...
        if let Some(pos) = raw_path.find('?') {
            ...
            let raw_query = &raw_path[pos + 1..];
            let query =
                raw_query.trim_left_matches('<').trim_right_matches('>').to_string();
